...
def get_legend_anchors(drawings: list) -> dict[str, float] | None:
    ...
    for d in drawings:
        for it in d['items']:
            if it[0] == 'l':
                p1, p2 = it[1], it[2]
                if 716 < p2.x < 722 and 708 < p1.x < 719:
                    pts.append(round(p2.y, 1))
                if 716 < p1.x < 722 and 708 < p2.x < 719:
                    pts.append(round(p1.y, 1))
    pts = sorted(set(pts))
...
def assign_all_curves(pdf_path: str) -> tuple[CurveResult, CurveResult, list[str]]:
    ...
    # used for assignment. Legend-anchor matching was tried first but proved fragile
    # for closely-spaced labels (greedy nearest-anchor matching occasionally swapped
    # adjacent pairs like 30/40 or 15/20 mS/m) and was abandoned in favor of the
    # rank-order approach below.
    #
    # DECISION (deliberately not implemented): using the legend as an independent
    # cross-check against rank-order - flagging a warning if the two methods would
    # have assigned labels differently - was considered and explicitly deferred.
    # Rank-order alone has been validated against all 20 real FCC source PDFs and
    # passes cleanly (see tests/test_digitizer.py::test_all_20_frequencies), so the
    # cross-check isn't needed for current correctness. It would add defensive value
    ...
    if len(top_candidates) == 17:
        ...
        raise ValueError(
            f"Top panel: found {len(top_candidates)} candidate curves, expected 17 or 16"
        )
